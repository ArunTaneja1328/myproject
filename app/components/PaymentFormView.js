...
export default class PaymentFormView extends React.Component {
  ...
  render() {
    ...
    return (
      <View>
        <View>
          <CreditCardInput labels={{expiry: strings('Misc.expiry'), name:strings('Misc.cardHolderName'), number:strings('Misc.cardNumber'), cvc:strings('Misc.cvc'), postalCode:strings('Misc.postalCode')}} requiresName onChange={(cardData) => this.setState({ cardData })} />
        </View>
        <View style={[styles.buttonWrapper, {marginTop: 20}]}>
          <Button
            title={strings('Payment.pay')}
